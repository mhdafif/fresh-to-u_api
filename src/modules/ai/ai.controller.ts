...
const identifyQuerySchema = z.object({
  ...
  locale: z.enum(["id", "en"]).default("en"),
  type: z.enum(["SCAN", "SEARCH"]).default("SEARCH"),
});
...
export class AIController {
  ...
  static async identifyByQuery(req: AuthenticatedRequest, res: Response) {
    try {
      const validatedData = identifyQuerySchema.parse(
        req.body
      ) as AIIdentifyQueryRequest;
      const language = getLanguage(req);
      ...
      const { text } = await generateText({
        ...
        messages: [
          {
            role: "user",
            content: `Identify the food based on this query and return the result in valid JSON format. Query: "${validatedData.query}". Respond in locale ${language}.`,
          },
        ],
        temperature: 0.1,
        maxRetries: 2,
